...
public class UGVuforia {
    ...
    public ringPos GetPosition(boolean saveBitmaps, boolean red) {
        Image rgbImage = null;
        ...
        if (rgbImage != null) {
            ...
            bitmapName = "imageRing.png";
            croppedBitmapName = "croppedRing.png";
            ...
            if (red) {
                ...
                cropStartX = (int) ((120.0 / 640.0) * bitmap.getWidth());
                cropStartY = (int) ((240.0 / 480.0) * bitmap.getHeight());
                cropWidth = (int) ((360.0 / 640.0) * bitmap.getWidth());
                ...
                cropStartX = (int) ((20.0 / 640.0) * bitmap.getWidth());
                cropStartY = (int) ((300.0 / 480.0) * bitmap.getHeight());
                cropWidth = (int) ((360.0 / 640.0) * bitmap.getWidth());
                cropHeight = (int) ((120.0 / 480.0) * bitmap.getHeight());
            }
            ...
            bitmap = createBitmap(bitmap, cropStartX, cropStartY, cropWidth, cropHeight); //Cropped Bitmap to show only rings
            ...
            for (height = 0; height <= bitmapHeight; height++) {
                for (width = 0; width <= bitmapWidth; width++) {
                    ...
                    if (Color.red(pixel) < 200 || Color.green(pixel) < 200 || Color.blue(pixel) < 200) {
                        ...
                        //r value so you dont gget exact red
                    }
                }
            }
...
//            }
        ...
        double yellowBlackRatioC = yellowCountC / blackCountC;
        //double blackYellowRatioR = blackCountR / yellowCountR;

        ringPos pos;
        if (yellowBlackRatioC > 1.0) {
            pos = ringPos.FOUR;
        } else if (yellowBlackRatioC > 0.2) {
            pos = ringPos.ONE;
        } else {
            pos = ringPos.NONE;
        }
